...
class Ensemble(nn.Module):
    ...
    def __init__(self, BasicBlock, BasicClfBlock,
                 num_classes=10, input_size = (3,32,32), iterations = 8, width = 128, lr=1.):
        super(Ensemble, self).__init__()

        # add layers to dictionary
        self.standby = {0: BasicBlock(input_size, width, residual = False,scale=lr)}
        for i in range(1,iterations):
            self.standby[i] = BasicBlock(self.standby[i-1].sizeof_output(), width=width, residual = True, scale=lr)
            if i%6==0:
                lr /= 2.0
    ...
    def state_dict(self, destination=None, prefix='', keep_vars=False):
        ...
        for idx in self.standby:
            self.standby[idx].state_dict(destination, prefix + str(idx) + '.', keep_vars=keep_vars)
            self.standby_cf[idx].state_dict(destination, prefix + str(idx) + '_cf.', keep_vars=keep_vars)
...
def train(train_loader, test_loader, BasicBlock, BasicClfBlock,
    ...
    model = Ensemble(BasicBlock=BasicBlock, BasicClfBlock = BasicClfBlock, num_classes=num_classes,
                     input_size=input_size, iterations=iterations, width = width, lr=greedy_lr)
